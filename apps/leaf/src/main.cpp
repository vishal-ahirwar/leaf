...
{
    commands["create"] = {"Generates a new, fully structured Leaf project in a new directory.",
                          create};
    commands["clean"]  = {
        "Deletes all build files and temporary artifacts from the project directory.", clean};
    commands["format"] = {
        "Automatically formats all C++ source code files according to a consistent style.", format};
    commands["install"] = {"Fetches and installs all the dependencies listed in the conanfile.py.",
                           install};
    commands["help"]  = {"Displays a list of all available commands and their descriptions.", help};
    commands["build"] = {"A comprehensive command to compile the entire project and its "
    ...
    std::ranges::for_each(args,
                          [&](const auto& command)
                          {
                              if (auto function = commands.find(command);
