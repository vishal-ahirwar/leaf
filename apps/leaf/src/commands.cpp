...
int create()
{
    ...
    if (std::ranges::any_of(project_name, [](const char c) { return c == ' '; }))
    {
        fmt::print(fmt::emphasis::bold | fmt::fg(fmt::color::red),
                   "Error, Project name can't have space\n");
        return 1;
    };
    try
    {
        ...
        std::vector<std::string> files{
            fs::path(project_name) / "apps" / project_name / "src" / "main.cpp",
            fs::path(project_name) / "apps" / project_name / "CMakeLists.txt",
            fs::path(project_name) / "CMakeLists.txt",
            fs::path(project_name) / "conanfile.py",
            fs::path(project_name) / ".clangd",
            fs::path(project_name) / ".clang-format",
